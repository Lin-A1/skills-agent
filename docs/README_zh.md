...
### 2026-01-08
- ✅ **会话持久化**: 刷新页面后保持会话，自动恢复对话历史（基于 localStorage）。
- ✅ **对话轮数计算优化**: 4 轮阈值改为统计 user 消息数（1 轮 = 1 条 user + 对应的 assistant/tool），工具调用不再影响阈值判断。
- ✅ **LLM 知识提取**: 记忆检索第二阶段从"选序号"升级为"LLM 直接整理知识"，返回可直接使用的信息而非原始消息。
- ✅ **WebSearch 修复**: 修复 OCR 和 SearXNG 容器网络连接问题。
