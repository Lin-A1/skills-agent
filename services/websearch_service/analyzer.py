...
class IntelligentSearchAnalyzer:
    ...
    LLM_ANALYSIS_PROMPT_TEMPLATE = """你是专业的网页内容分析助手，负责为联网搜索工具提取结构化信息。

        【任务】分析网页文本内容，提取与搜索词相关的核心信息
        【搜索词】{query}
        ...
        2. main_content (300-500字)
        - 全面概述页面核心内容
        - 包含：背景介绍、主要观点、关键数据、结论
        - 保持客观，不要添加推测
        ...
        4. credibility (来源可信度)
        - authoritative: 政府、学术、官方文档、知名机构
        - commercial: 企业官网、产品介绍、营销内容
        - forum: 论坛、问答、个人博客、社交媒体
        - unknown: 无法判断来源类型

        5. relevance_score (0.0-1.0 相关性评分)
        - 0.9-1.0: 直接回答搜索问题，信息权威完整
        - 0.7-0.9: 高度相关，包含有价值的关键信息
        - 0.5-0.7: 部分相关，有一定参考价值
        - 0.3-0.5: 边缘相关，仅有少量相关信息
        - 0.0-0.3: 不相关或信息过时/错误

        6. cacheable (是否适合长期缓存)
        - true: 内容稳定，适合缓存
          - 知识性内容（技术文档、百科、教程）
          - 历史事件、人物介绍
          - 概念定义、原理解释
